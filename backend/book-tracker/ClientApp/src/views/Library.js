...
export const LibraryComponent = () => {
  ...
  const Cell = ({ Book, Field, style }) => <div style={style}>Book[Field]</div>;
  ...
  const headerRenderer = ({ label, dataKey, rowData }) => {
    let direction =
      dataKey !== sortBy
        ? SortDirection.ASC
  ...
  const ImageCell = ({ cellData }) => <img height='100px' src={cellData} />;

  return (
    <>
      <div className='mb-5'>
        <h1>User Library</h1>
        <p className='lead'>View Books Added to your Library.</p>

        <p>
          Your library consist of any book added to a collection or marked as
          read. You can add books to your library by using the search option at
          the top. Once a book is added, you can mark it as read or add it to a
          collection for future use.
        </p>

        <LinkButton color='primary' className='mt-5' to='Books'>
          Add New Book
        </LinkButton>
        <LinkButton color='primary' className='mt-5' to='collection/new'>
          Add New Collection
        </LinkButton>
        ...
        >
          <option key='none' name='none' value='null'>
            None
          </option>
